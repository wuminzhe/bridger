...
use crate::{
    Result, LogsHandler, EthlikeChain, EthlikeClient
};
use crate::default_logs_handler::DefaultLogsHandler;

pub struct EthlikeTracker<C: EthlikeChain, H: LogsHandler> {
...
impl<C: EthlikeChain, H: LogsHandler> EthlikeTracker<C, H> {
    pub fn new(client: EthlikeClient, topics_list: Vec<(H160, Vec<H256>)>, logs_handler: H, from: u64) -> Self {
        EthlikeTracker {
            client,
            topics_list,
            logs_handler,
...
async fn test_heco() {
    ...
    let client = EthlikeClient::new(web3);
    let mut tracker = EthlikeTracker::<Heco, DefaultLogsHandler>::new(
        client,
        vec![(contract_address, topics)],
        DefaultLogsHandler {},
        4006177,
